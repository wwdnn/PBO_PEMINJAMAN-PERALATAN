...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('petugas_peralatan', function (Blueprint $table) {
            $table->id();
            $table->string('nama_petugas', 30)->unique()->comment('Nama petugas peralatan')->nullable(false);
            $table->string('username', 30)->unique()->comment('Username petugas peralatan')->nullable(false);
            $table->string('password', 200)->unique()->comment('Password petugas peralatan')->nullable(false);
            $table->timestamps();
        });
    }
